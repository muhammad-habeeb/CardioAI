...
def make_contribution_chart(model, input_dict):
    coefs = model.coef_[0]
    contributions = [coef * input_dict[name] for name, coef in zip(FEATURE_NAMES, coefs)]
    ...
    colors = ["#b91c1c" if value > 0 else "#15803d" for value in df["Contribution"]]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("B.Sc Computer Science Project · NHANES Dataset · Streamlit")
...
if not LOAD_OK:
    st.error(
        ...
        Make sure `heart_attack_model.pkl` and `feature_names.pkl` are inside the `models/` folder.
...
with right:
    ...
    with st.container(border=True):
        st.markdown("### 📋 Summary")
        chips_html = "".join(
            f'<div class="chip"><b>{FIELD_CONFIG[f]["label"]}:</b> {input_values[f]} {FIELD_CONFIG[f]["unit"]}</div>'
...
# ------------------------------------------------------------------
if predict_clicked:
    ordered_input = np.array([[input_values[f] for f in FEATURE_NAMES]])
    ...
    with res_col1:
        ...
        if level == "High":
            st.warning(
                "The model estimates elevated risk. Consider consulting a healthcare professional and reviewing lifestyle factors."
            )
        elif level == "Moderate":
            st.info(
                "The model estimates moderate risk. Regular checkups and healthy habits are recommended."
            )
        else:
            st.success("The model estimates low risk based on the provided values.")
    ...
    st.caption(
        "Bars show each factor's estimated push toward higher risk or lower risk based on the model."
    )
    st.plotly_chart(make_contribution_chart(model, input_values), use_container_width=True)

else:
    st.info("👈 Fill in patient details and click **Predict Risk** to see the result.")
...
# ------------------------------------------------------------------
st.markdown("---")
st.caption("Built with Streamlit · Logistic Regression Model · For educational purposes only.")
